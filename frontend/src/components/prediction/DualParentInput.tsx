...
export function DualParentInput({ onPredict, isLoading }: DualParentInputProps) {
  ...
  const ParentForm = ({ num, data }: { num: 1 | 2; data: ParentData }) => (
    <Card className="border-cyan-900/40 bg-slate-950/50 backdrop-blur-xl">
      <CardHeader>
        <CardTitle className="flex items-center text-cyan-400 gap-2 text-xl">
          <User className="h-5 w-5" /> Parent {num} {num === 1 ? "(Father)" : "(Mother)"}
        </CardTitle>
        <CardDescription>Select genotypes and blood group</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <h4 className="flex items-center gap-2 font-medium text-slate-300">
            <Droplet className="h-4 w-4 text-red-400" /> Blood Group & Rh Factor
          </h4>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>Blood Group (ABO)</Label>
              <Select value={data.blood} onValueChange={(v) => handleUpdate(num, "blood", v)}>
            ...
            <div className="space-y-2">
              ...
              <Select value={data.rh} onValueChange={(v) => handleUpdate(num, "rh", v)}>
                <SelectTrigger className="bg-slate-900 border-slate-700"><SelectValue /></SelectTrigger>
                <SelectContent>
        ...
        <div className="space-y-4">
          <h4 className="flex items-center gap-2 font-medium text-slate-300">
            <Activity className="h-4 w-4 text-emerald-400" /> Genetic Markers
          </h4>
          
          <div className="space-y-2">
            <Label>Beta Thalassemia (HBB gen.) - Chr 11</Label>
            <Select value={data.genotypes.beta_thalassemia} onValueChange={(v) => handleUpdate(num, "beta_thalassemia", v, true)}>
              <SelectTrigger className="bg-slate-900 border-slate-700"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select value={data.genotypes.sickle_cell} onValueChange={(v) => handleUpdate(num, "sickle_cell", v, true)}>
              <SelectTrigger className="bg-slate-900 border-slate-700"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="AA">Healthy (AA)</SelectItem>
          ...
          <div className="space-y-2">
            ...
            {num === 1 ? (
              <Select value={data.genotypes.g6pd_deficiency} onValueChange={(v) => handleUpdate(num, "g6pd_deficiency", v, true)}>
                <SelectTrigger className="bg-slate-900 border-slate-700"><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            ...
            {num === 1 ? (
              <Select value={data.genotypes.y_chromosome_infertility} onValueChange={(v) => handleUpdate(num, "y_chromosome_infertility", v, true)}>
                <SelectTrigger className="bg-slate-900 border-slate-700"><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              <Select disabled value="normal">
                <SelectTrigger className="bg-slate-900/50 text-slate-500 border-slate-800"><SelectValue placeholder="Not applicable (Y-Linked)" /></SelectTrigger>
    ...
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div className="flex justify-center">
        <button
          ...
          disabled={isLoading}
          className="relative inline-flex h-14 w-64 overflow-hidden rounded-full p-[2px] focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-slate-50 transition-transform active:scale-95 disabled:scale-100 disabled:opacity-70"
        >
          <span className="absolute inset-[-1000%] animate-[spin_3s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#0891b2_0%,#06b6d4_50%,#22d3ee_100%)]" />
          <span className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-slate-950 px-8 py-1 text-lg font-bold text-white backdrop-blur-3xl tracking-widest uppercase">
